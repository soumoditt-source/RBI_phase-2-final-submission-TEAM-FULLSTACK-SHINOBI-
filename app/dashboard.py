...
def load_submission() -> pd.DataFrame:
    # Check results folder first (priority for processed artifacts), then root
    # Priority: .pkl (Rule-compliant) -> .csv (Original)
    for ext in [".pkl", ".csv"]:
        for candidate in [RESULTS / f"submission{ext}", DATA_DIR / f"submission{ext}"]:
            if candidate.exists():
                try:
                    if ext == ".pkl": 
                        df = pd.read_pickle(candidate)
                    else:
                        df = pd.read_csv(candidate, dtype={"account_id": str, "is_mule": float})
...
@st.cache_data(ttl=900, show_spinner="👮 Loading Forensic Violations ...")
def load_violations() -> pd.DataFrame:
    # V12.7.1 Supreme: Ultra-Robust Hybrid Loader (.pkl fallback for rule compliance)
    for ext in [".pkl", ".parquet"]:
        for candidate in [RESULTS / f"account_violations{ext}", DATA_DIR / f"account_violations{ext}"]:
            if candidate.exists():
                try:
                    df = pd.read_pickle(candidate) if ext == ".pkl" else pd.read_parquet(candidate)
                    if "account_id" in df.columns:
                        df["account_id"] = df["account_id"].astype(str).str.strip()
                    return df
                except Exception as e:
                    st.warning(f"Forensic Kernel Error: {e}")
    return pd.DataFrame()
...
raw_viols   = load_violations() 
shap_df     = load_shap()

# Standardize violations for UI (V12.6 Consolidated)
violations = violations_to_df(raw_viols)
viols_df = violations # Already mapped in violations_to_df
...
# Safe violations summary — Optimized DataFrame operations
...
with tabs[6]:
    ...
    if viols_df.empty:
        ...
        # Search HUD (V12.7 Fixed)
        c_search, c_band, c_sort = st.columns([2, 1, 1])
        with c_band:
            filter_band = st.selectbox("Filter Risk Band", ["ALL", "CRITICAL", "HIGH", "MEDIUM", "LOW"], key="dive_filter_band_v12")
        with c_sort:
            sort_by = st.selectbox("Sort By", ["Risk Score (High-Low)", "Risk Score (Low-High)", "Account ID"], key="dive_sort_by_v12")
        with c_search:
            def on_dive_search():
                st.session_state["focus_id"] = st.session_state["dive_search_input"].strip()

            st.text_input("🔍 Forensic Identity Search", 
                         value=st.session_state["focus_id"],
                         key="dive_search_input",
                         on_change=on_dive_search,
                         placeholder="Enter Account ID...").strip()
            search_id = st.session_state["focus_id"]

        # Super-Resolution Filtering (V12.3 Capped for performance)
        acc_df = viols_df.copy()
        if filter_band != "ALL":
            acc_df = acc_df[acc_df[COL_RISK_BAND] == filter_band]
        
        if search_id:
            # V12.5 SUPREME SEARCH: Liquid-Fast Identity Coverage
            search_str = str(search_id).strip()
            # Priority 1: Exact Match (Fastest)
            exact_match = acc_df[acc_df[COL_ACC_ID].astype(str) == search_str]
            if not exact_match.empty:
                acc_df = exact_match
            else:
                # Priority 2: Fuzzy Containment (Capped for performance)
                acc_df = acc_df[acc_df[COL_ACC_ID].astype(str).str.contains(search_str, case=False, na=False)].head(100)
        
        # Sort and cap unique list
        if sort_by == "Risk Score (High-Low)":
            acc_df = acc_df.sort_values(COL_RISK_SCORE, ascending=False)
        elif sort_by == "Risk Score (Low-High)":
            acc_df = acc_df.sort_values(COL_RISK_SCORE, ascending=True)
        else:
            acc_df = acc_df.sort_values(COL_ACC_ID)
        
        total_matches = len(acc_df[COL_ACC_ID].unique())
        acc_list = acc_df[COL_ACC_ID].unique()[:500] # ULTRA-CAP to prevent browser lag

        if len(acc_list) == 0:
            st.warning("No records match your forensic search.")
        else:
            # If focus_id is set but not in current limited list, add it to the top
            current_focus = st.session_state.get("focus_id", "")
            final_acc_list = list(acc_list)
            if current_focus and current_focus not in final_acc_list:
                # Check if it actually exists in the full dataset
                if current_focus in viols_df[COL_ACC_ID].values:
                    final_acc_list.insert(0, current_focus)

            selected_acc = st.selectbox(
                f"Select Target Forensic Dossier ({total_matches:,} matching, showing top 1000)",
                final_acc_list,
                key="dive_acc_selector"
            )
            if selected_acc != st.session_state["focus_id"]:
                st.session_state["focus_id"] = selected_acc
                st.rerun()

            # ─── Dossier Rendering (High-Definition) ───
            acc_viols = viols_df[viols_df[COL_ACC_ID] == selected_acc]
            if not acc_viols.empty:
                top_v = acc_viols.iloc[0]
                band_color = SEVERITY_COLORS.get(top_v[COL_SEVERITY], "#8b9dc3")
                score = top_v[COL_RISK_SCORE]
                ...
                <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                        <div>
                            ...
                            <div style="font-size:0.8rem;color:#8b9dc3;margin-top:4px;">Official Audit | RBI NFPC Phase 2</div>
                        </div>
                        <div style="text-align:right;">
                            ...
                            <div style="margin-top:6px;">
                                <span style="background:rgba(255,255,255,0.07);color:{band_color};
                                    ...
                                    {top_v[COL_SEVERITY]} RISK
                                </span>
                                <span style="background:rgba(255,140,66,0.07);color:#ff8c42;
                                             border:1px solid #ff8c42;border-radius:20px;
                                             padding:4px 14px;font-size:0.8rem;font-weight:700;margin-left:8px;">
                                    {top_v.get(COL_SEGMENT, "STANDARD")}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                # --- 17GB+ Feature Depth Expansion (NEW) ---
                with st.expander("🔬 Specialized Forensic Feature Depth (HFT-Cortex)", expanded=True):
                    f1, f2, f3 = st.columns(3)
                    # Extract HFT features from top_v (which now has them all thanks to V12.4 merge)
                    f1.metric("Transaction Velocity", f"{top_v.get('hft_txn_count', 0):.0f}")
                    f2.metric("Layering Index", f"{top_v.get('hft_x_layered_cash_index', 0):.3f}")
                    f3.metric("Geography Drift", f"{top_v.get('geo_max_drift_km', 0):.1f} km")
                    
                    st.markdown("<br>", unsafe_allow_html=True)
                    # Show more features in a small table if present
                    hft_cols = [c for c in top_v.index if c.startswith('hft_') or c.startswith('geo_')]
                    if hft_cols:
                        feat_data = {c: top_v[c] for c in hft_cols[:12]} # selected top 12 for impact
                        st.json(feat_data)

                st.markdown(f"""
                <div style="background:rgba(255,255,255,0.02);border-radius:12px;padding:20px;margin-top:16px;">
                    <hr style="border-color:rgba(255,255,255,0.06);margin:0 0 16px 0;">
                    <div style="font-size:0.9rem;color:#f0f4ff;line-height:1.6;">{top_v.get(COL_EVIDENCE,'Multiple statutory breaches identified.')}</div>
                    <div style="margin-top:14px;background:rgba(255,51,102,0.07);border:1px solid rgba(255,51,102,0.2);
                        ...
                        <strong>ENFORCEMENT ACTION:</strong><br>{top_v.get(COL_REQ_ACTION,'Enhanced Due Diligence required.')}
                    </div>
                </div>
                """, unsafe_allow_html=True)
                
                # Individual Violation Breakdown
                st.markdown(f"### {len(acc_viols)} Statutory Violation(s) Detected")
                for _, v in acc_viols.iterrows():
                    v_color = SEVERITY_COLORS.get(v[COL_SEVERITY], "#8b9dc3")
                    st.markdown(f"""
                    <div style="background:rgba(255,255,255,0.02);border:1px solid {v_color}33;
                        ...
                        <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                            <div style="font-weight:700;color:#f0f4ff;font-size:1.0rem;">{v[COL_VIOLATION]}</div>
                            <div style="font-size:0.75rem;color:{v_color};font-weight:700;">{v[COL_SEVERITY]}</div>
                        </div>
                        <div style="font-size:0.82rem;color:#8b9dc3;margin-bottom:8px;"><i>Statute: {v[COL_STATUTE]}</i></div>
                        <div style="font-size:0.85rem;color:#f0f4ff;">{v[COL_EVIDENCE]}</div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                st.download_button(
                    ...
                    data=acc_viols.to_json(orient="records"),
                    file_name=f"CASE_{selected_acc}.json",
                    mime="application/json",
                    key="dive_download_jsonv12"
                )
            else:
